  ...
  h.onMessage([&pid](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
//          std::ofstream logfile("log.csv", std::ios_base::app | std::ios_base::out);
          ...
          */
          double dist_inc = speed * dt_sec;
          pid.UpdateError(cte, dt_sec);
          steer_value = pid.TotalError();
          //pid.Twiddle(cte, dist_inc);
          
          // DEBUG
//          std::cout << "CTE: " << cte << " Steering Value: " << steer_value <<  " dt: " << dt_sec << std::endl;
...
//          logfile << "dt," << dt_sec << ",time," << time_sec << ",cte," << cte << ",steer_angle_act," << angle << ",steer_angle_command," << steer_value << ",speed," << speed << "\n";
//          logfile << dt_sec << "," << time_sec << "," << cte << "," << angle << "," << steer_value << "," << speed << "\n";
//          logfile.close();
        }
      } else {
        // Manual driving
